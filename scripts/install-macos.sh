...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║              Senterm macOS Installer                         ║${NC}"
...
# Check architecture
ARCH=$(file "${SOURCE_BINARY}" | grep -o 'arm64\|x86_64' | head -1)
if file "${SOURCE_BINARY}" | grep -q "universal"; then
    echo -e "${GREEN}✓${NC} Universal binary (x86_64 + arm64)"
else
